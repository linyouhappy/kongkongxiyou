 ...
 * @param {Function} cb
 */
taskDao.getTasksByPlayerId = function(playerId, cb) {
...
taskDao.updateTaskState = function(task, cb) {
	var sql = 'update Task set taskState = ?, startTime = ? where id = ?';
	var args = [task.taskState, task.startTime, task.id];	
	pomelo.app.get('dbclient').query(sql, args, function(err, res) {
		if (!!err) {
			logger.error('update MainTask for mainTaskDao failed!' + err.stack);
			utils.invokeCallback(cb, err);
		} else {
...
taskDao.updateTaskData=function(task,cb){
	var sql = 'update Task set targetCount = ? where id = ?';
	var args = [task.targetCount, task.id];	
	pomelo.app.get('dbclient').query(sql, args, function(err, res) {
		if (!!err) {
			logger.error('update MainTask for mainTaskDao failed!' + err.stack);
			utils.invokeCallback(cb, err);
...
taskDao.getCurTasksByPlayId = function(playerId, cb) {
	...
	pomelo.app.get('dbclient').query(sql, args, function(err,res) {
		if (err) {
			...
			for (var i = 0; i < length; i ++) {
				var task = createNewTask(res[i]);
				if (task.taskState === consts.TaskState.NOT_COMPLETED 
					|| task.taskState === consts.TaskState.COMPLETED_NOT_DELIVERY){
					tasks[task.id] = task;
				}
			}
			utils.invokeCallback(cb, null, tasks);
 ...
 * @param {Function} cb
 */
taskDao.createTask = function(task, cb) {
	var sql = 'insert into Task (playerId, kindId,taskState,startTime) values (?, ?, ?, ?)';
	var args = [task.playerId, task.kindId,task.taskState,task.startTime];
	pomelo.app.get('dbclient').insert(sql, args, function(err, res) {
		if (!!err) {
			...
			utils.invokeCallback(cb, null, taskData);
		}
	});
};

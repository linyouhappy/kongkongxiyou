...
pro.entryPlayer = function(msg, session, next) {
	...
	async.waterfall([
		...
		function(res, cb) {
			...
			var args = [player.kindId, uid];
			app.get('dbclient').query(sql, args, function(err, res) {
				if (err !== null) {
					logger.error('update user lastKindId failed!');
				} else {
		...
		if (rpc.chat) {
			rpc.chat.chatRemote.add(session, player.userId, player.id,
				channelUtil.getGlobalChannelName(), BlankFunc);
		}
		if (rpc.manager) {
			rpc.manager.guildRemote.enterGuild(session, player.userId, player.guildId, player.id, BlankFunc);
			// rpc.manager.guildRemote.enterGuild(session, player.userId, player.guildId, player.id, function(err,guildId) {
			// 	if (!err) {
			// 		if (player.guildId!==guildId) {
			// 			utils.myPrint("ERROR:player.guildId=",player.guildId,",guildId=",guildId);

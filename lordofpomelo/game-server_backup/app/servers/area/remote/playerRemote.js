...
exp.kickPlayers=function(playerIds,cb){
  ...
  for (var i = 0; i < playerIds.length; i++) {
    var player = area.getPlayer(playerIds[i]);
    if (player) {
...
exp.leaveInstance = function(args, cb) {
  ...
  var area = areaManager.getArea(args.instanceId);
  if (!area) {
  ...
  if (player) {
    // logger.error("ERROR:player is exist!!!");
    area.exitArea(player);
  }
  if (area.type > AreaType.SCENE) {
    if (area.isEmpty()) {
      instancePool.removeInstance(args.instanceId);
    }
  }
  utils.invokeCallback(cb);
};
...
exp.playerLeave = function(args, cb) {
  ...
  if (area.instanceId && area.isEmpty()) {
    instancePool.removeInstance(args.instanceId);
    logger.info("playerLeave removeInstance instanceId=", args.instanceId);
  }
  utils.invokeCallback(cb);
};

...
var AreaDomainLogic = function(opts) {
  ...
  this.occupyTime=Math.floor(Date.now()/1000);;
};
...
AreaDomainLogic.prototype.initDomainArea = function() {
  ...
  this.domain = {};
  var guildTown = dataApi.guild_town.findById(this.areaId % 1000);
  if (guildTown) {
    var deltaX = 150;
    var deltaY = 80;
    this.domain.guildTown = guildTown;
    this.domain.minX = guildTown.pointX - deltaX;
    this.domain.maxX = guildTown.pointX + deltaX;
    this.domain.minY = guildTown.pointY - deltaY;
    this.domain.maxY = guildTown.pointY + deltaY;
  }
};
...
AreaDomainLogic.prototype.reviveInArea = function(player) {
  ...
  if (player.isRevive) {
    ...
    if (this.occupyGuildId && player.guildId === this.occupyGuildId) {
      var oldPos = {
        x: player.x,
        y: player.y
      };
  ...
  setTimeout(function() {
    ...
    if (player) {
      ...
      var msg = {
        ...
        guildId: self.occupyGuildId,
        value:self.occupyValue,
        level: Math.ceil((currentTime-self.occupyTime)/60)
      };
...
AreaDomainLogic.prototype.pushDomainInfo = function() {
  ...
  var msg = {
    ...
    guildId: this.occupyGuildId,
    value:this.occupyValue,
    level: Math.ceil((currentTime-this.occupyTime)/60)
  };
...
AreaDomainLogic.prototype.updateDomainBattle = function() {
  ...
  var entity, guild, maxCountGuildId;
  ...
  if (this.areaState !== AreaStates.BATTLE_STATE) {
    this.areaState = AreaStates.BATTLE_STATE;
    this.pushDomainInfo();
  }
  var occupyValue=this.occupyValue;
  if (!this.occupyGuildId) {
    ...
    if (guild) {
      currentCount = guild.length;
      var otherCount = totalCount - currentCount;
      if (otherCount === 0) {
    ...
    occupyValue += currentCount * 30;
    if (occupyValue >= 1000) {
      occupyValue = Math.min(1000, occupyValue);
    } else if (occupyValue > 0) {
      occupyValue = Math.min(1000, occupyValue);
    } else if (occupyValue <= 0) {
      var otherCount = totalCount - maxCount;
      var deltaCount = maxCount - otherCount;
  ...
  // if (occupyValue!==this.occupyValue) {
...
//   var self = this;
//   handleRemote.guildRemote_finishDomain(null,this.areaId,domain.guildId,domain.level,function(err, guildName) {
//     var msg;
//     if (guildName) {
...
AreaFightLogic.prototype.finishFight = function(loserId) {
  ...
  var self = this;
  pomelo.app.rpc.manager.fightRemote.finishFight(null, winnerId, loserId, function(err, fightLevel) {
    if (fightLevel) {
...
AreaMyBossLogic.prototype.enterArea = function(player) {
  ...
  // this.area.map.genPatrolPath(mob,500,250);
  // mob.area=this.area;
...
/////////////////////////////////////////////
//AreaWorldBossLogic
var AreaWorldBossLogic = function(opts) {
  AreaLogic.call(this, opts);
};

util.inherits(AreaWorldBossLogic, AreaLogic);

AreaWorldBossLogic.prototype.enterArea = function(player) {
  logger.info("AreaWorldBossLogic enterArea=======>>");
...
AreaWorldBossLogic.prototype.exitArea = function(player) {
  logger.info("AreaWorldBossLogic exitArea=======>>");
  // var instanceId=this.area.instanceId;
  // process.nextTick(function(){
  //   if (pomelo.app.areaManager.removeInstance) {
  //     pomelo.app.areaManager.removeInstance(instanceId);
...
// AreaWorldBossLogic.prototype.diedInArea = function(player) {
//   logger.info("AreaWorldBossLogic diedInArea=======>>");
// };

AreaWorldBossLogic.prototype.reviveInArea = function(player) {
  logger.info("AreaWorldBossLogic reviveInArea=======>>");
  this.area.timer.abortAllAction(player.entityId);
  if (player.isRevive) {
    player.onRevive();
  } else {
...
AreaWorldBossLogic.prototype.startLogic = function() {
  logger.info("AreaWorldBossLogic startLogic=======>>");
  this.area.start();
};

// AreaWorldBossLogic.prototype.killReward = function(attacker, target, msg, items) {
//   if (target.type === EntityType.MOB) {
//     if (this.myBossData.bossId === target.kindId) {
//       var taskItems = this.myBossData.items;
//       if (taskItems) {
...
areaLogicsList[AreaKinds.WORLD_BOSS_AREA]=AreaWorldBossLogic;

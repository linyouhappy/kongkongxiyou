...
handler.enterScene = function(msg, session, next) {
  ...
  var playerId = session.get('playerId');
  // var instanceId = session.get('instanceId');
  utils.myPrint("EnterScene areaId =", area.areaId, ",playerId=", playerId);
...
handler.changeArea = function(msg, session, next) {
  ...
  // areaManager.changeArea(msg, session, function(code,instanceId) {
  //   if (code) {
  //     if (code == 200) {
  //       if (area.instanceId) {
...
handler.getProperty = function(msg, session, next) {
  ...
  if (playerId) {
    ...
    var entity=areaManager.getCanUsedPlayer(playerId);
    // var entity=area.getPlayer(playerId);
    if (entity) {
      var msg = {
        kindId: entity.kindId,
        level: entity.level,
        caoCoin: entity.caoCoin,
        // maxHp: entity.maxHp,
        playerId: entity.id,
        name: entity.name,

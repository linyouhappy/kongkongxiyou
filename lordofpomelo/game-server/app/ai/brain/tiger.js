...
var MoveToTargetForAttack = require('../action/moveToTargetForAttack');
...
var Brain = function(blackboard) {
	...
	var attack = new TryAndAdjust({
		...
		tryAction: new TryAttack({
			...
			// 	return bb.curCharacter.normalSkill; //normal attack
			// }
		})
	});
	...
	var checkTarget = function(bb) {
		// console.log("AI checkTarget=============>>");
		if(bb.curTarget !== bb.curCharacter.target) {
			// console.log("AI checkTarget=============>>target change");
			// target has change
			bb.curTarget = null;
			return false;
		}
	...
	var haveTarget = function(bb) {
		...
		// console.log("AI haveTarget=============>>!EntityType.PLAYER");
		return false;
	};
	...
	var noTarget = function(bb) {
		// console.log("AI noTarget=============>>");
		var character = bb.curCharacter;
		var target = bb.area.getEntity(character.target);
		if(target) {
			return false;
		}
		return true;
	};
	var patrolIfNoTarget = new If({
		blackboard: blackboard, 
		cond: noTarget, 
		action: patrol
	});
	...
	this.action.addChild(patrolIfNoTarget);
};

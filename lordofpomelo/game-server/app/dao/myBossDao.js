...
myBossDao.create = function(playerId,cb) {
	...
	// var sql = 'insert into MyBoss (playerId,recordTime,times1,times2,times3,times4,times5) values (?,?,?,?,?,?,?)';
	// var args = [playerId, dayInt, 3, 3, 2, 2, 2];
...
myBossDao.update = function(value, cb) {
	var sql = 'update MyBoss set times1=?,times2=?,times3=?,times4=?,times5=? where id = ?';
	var args = [value.times1, value.times2, value.times3, value.times4, value.times5, value.id];
	pomelo.app.get('dbclient').query(sql, args, function(err, res) {
		if (err) {
